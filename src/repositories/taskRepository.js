...
async function findAll({ status, search, page = 1, limit = 20 }) {
  ...
  return {
    items: result.rows.map(({ totalCount, ...task }) => task),
...
async function update(id, { title, description, completed, priority, dueDate }) {
  ...
  if (description !== undefined) add('description', description.trim());
  if (completed !== undefined) add('completed', completed);
  if (priority !== undefined) add('priority', priority);
  if (dueDate !== undefined) add('due_date', dueDate || null);
